# Implementation of the filters is borrowed from paper "Convolutional Neural Operators for robust and accurate learning of PDEs" https://github.com/camlab-ethz/ConvolutionalNeuralOperator
...
from conv2d_nlt import Conv2d_nlt
from batchnorm import nlt_BatchNorm2d
...
class CNOBlock(nn.Module):
    def __init__(self,
        ...
        if nlt:
            self.convolution = Conv2d_nlt(in_channels=self.in_channels, out_channels=self.out_channels,
        ...
        if self.batch_norm:
            if nlt:
                self.batch_norm = nlt_BatchNorm2d(self.out_channels)
            else:
...
class LiftProjectBlock(nn.Module):
    def __init__(self,
        ...
        if nlt:
            self.convolution = Conv2d_nlt(in_channels=latent_dim, out_channels=out_channels,
        ...
        if self.batch_norm:
            if nlt:
                self.batch_norm = nlt_BatchNorm2d(out_channels)
            else:
...
class ResidualBlock(nn.Module):
    def __init__(self,
        ...
        if nlt:
            self.convolution1 = Conv2d_nlt(self.channels, self.channels, kernel_size=self.conv_kernel, stride=1,
                                           padding=pad)

            self.convolution2 = Conv2d_nlt(self.channels, self.channels, kernel_size=self.conv_kernel, stride=1,
                                           padding=pad)
        ...
        if self.batch_norm:
            if nlt:
                self.batch_norm1 = nlt_BatchNorm2d(self.channels)
                self.batch_norm2 = nlt_BatchNorm2d(self.channels)
            else:
