...
# from CNOModule_nlt import CNO as cno_nlt

from CNOModule_lora import CNO as cno_nlt
...
class APENSDataset(Dataset):
    def __init__(self, which="training", nf=0, training_samples=512, s=128, in_dist=True):

        # Note: Normalization constants for both ID and OOD should be used from the training set!
        # Load normalization constants from the TRAINING set:
        if in_dist:
            self.file_data = r"C:\Users\pangg\Desktop\apebench\data\NS_f2_5e-4_train.npy"
        else:
            self.file_data = r'C:\Users\pangg\Desktop\apebench\data\NS_f2_1e-4_train.npy'


        file_data_train = r"C:\Users\pangg\Desktop\apebench\data\NS_f2_5e-4_train.npy"
        train_data = np.load(file_data_train)
        self.min_data = train_data[:, 0, 0, :, :].min()
        self.max_data = train_data[:, 0, 0, :, :].max()
        self.min_model = train_data[:, -1, 0, :, :].min()
        self.max_model = train_data[:, -1, 0, :, :].max()

        self.reader = np.load(self.file_data)
    ...
    def __getitem__(self, index):
        inputs = torch.from_numpy(
            self.reader[index+self.start, 0, 0, :, :]).type(
            torch.float32).reshape(1, self.s, self.s)
        labels = torch.from_numpy(
            self.reader[index+self.start, -1, 0, :, :]).type(
            torch.float32).reshape(1, self.s, self.s)
...
class ApeNS:
    def __init__(self, network_properties, device, batch_size, training_samples=1024, s=128, in_dist=True, nlt=False):
        ...
        if nlt:
            self.model = cno_nlt(in_dim=1 + 2 * self.N_Fourier_F,  # Number of input channels.
                             in_size=self.in_size,  # Input spatial size
                             N_layers=N_layers,  # Number of (D) and (U) Blocks in the network
                             N_res=N_res,  # Number of (R) Blocks per level
                             N_res_neck=N_res_neck,
        ...
        self.train_loader = DataLoader(APENSDataset("training", self.N_Fourier_F, training_samples, s, in_dist),
                                       batch_size=batch_size, shuffle=True, num_workers=num_workers)
        self.val_loader = DataLoader(APENSDataset("validation", self.N_Fourier_F, training_samples, s, in_dist),
                                     batch_size=batch_size, shuffle=False, num_workers=num_workers)
        self.test_loader = DataLoader(APENSDataset("test", self.N_Fourier_F, training_samples, s, in_dist),
                                      batch_size=batch_size, shuffle=False, num_workers=num_workers)
...
class KSDataset(Dataset):
    def __init__(self, which="training", nf=0, training_samples=512, s=128, in_dist=True):
        ...
        if in_dist:
            self.file_data = r"C:\Users\pangg\Desktop\py-pde\KS_6.npy"
        else:
            self.file_data = r"C:\Users\pangg\Desktop\py-pde\KS_8.npy"

        file_data_train = r"C:\Users\pangg\Desktop\py-pde\KS_6.npy"
        train_data = np.load(file_data_train)
        self.min_data = train_data[:, 0, :, :].min()
        self.max_data = train_data[:, 0, :, :].max()
        self.min_model = train_data[:, 1, :, :].min()
        self.max_model = train_data[:, 1, :, :].max()
    ...
    def __getitem__(self, index):
        inputs = torch.from_numpy(
            self.reader[index+self.start, 0, :, :]).type(
            torch.float32).reshape(1, self.s, self.s)
        labels = torch.from_numpy(
            self.reader[index+self.start, 1, :, :]).type(
            torch.float32).reshape(1, self.s, self.s)
...
class KS:
    def __init__(self, network_properties, device, batch_size, training_samples=1024, s=128, in_dist=True, nlt=False):
        ...
        self.train_loader = DataLoader(KSDataset("training", self.N_Fourier_F, training_samples, s, in_dist),
                                       batch_size=batch_size, shuffle=True, num_workers=num_workers)
        self.val_loader = DataLoader(KSDataset("validation", self.N_Fourier_F, training_samples, s, in_dist),
                                     batch_size=batch_size, shuffle=False, num_workers=num_workers)
        self.test_loader = DataLoader(KSDataset("test", self.N_Fourier_F, training_samples, s, in_dist),
                                      batch_size=batch_size, shuffle=False, num_workers=num_workers)

class BrussDataset(Dataset):
    def __init__(self, which="training", nf=0, training_samples=512, s=128, in_dist=True):
        ...
        if in_dist:
            self.file_data = r"C:\Users\pangg\Desktop\py-pde\Brusselator_6.npy"
        else:
            self.file_data = r"C:\Users\pangg\Desktop\py-pde\Brusselator_8.npy"

        file_data_train = r"C:\Users\pangg\Desktop\py-pde\Brusselator_6.npy"
        train_data = np.load(file_data_train)
        self.min_data = train_data[:, 0, :, :].min()
        self.max_data = train_data[:, 0, :, :].max()
        self.min_model = train_data[:, 1, :, :].min()
...
class Bruss:
    def __init__(self, network_properties, device, batch_size, training_samples=1024, s=128, in_dist=True, nlt=False):
        ...
        self.train_loader = DataLoader(BrussDataset("training", self.N_Fourier_F, training_samples, s, in_dist),
                                       batch_size=batch_size, shuffle=True, num_workers=num_workers)
        self.val_loader = DataLoader(BrussDataset("validation", self.N_Fourier_F, training_samples, s, in_dist),
                                     batch_size=batch_size, shuffle=False, num_workers=num_workers)
        self.test_loader = DataLoader(BrussDataset("test", self.N_Fourier_F, training_samples, s, in_dist),
                                      batch_size=batch_size, shuffle=False, num_workers=num_workers)