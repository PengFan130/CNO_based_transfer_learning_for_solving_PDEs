...
class _ConvNd_nlt(Module):
    ...
    def reset_parameters(self):
        ...
        self.nlt_weight.data.uniform_(1, 1) # init as 1
        if self.bias is not None:
            self.bias.data.uniform_(-stdv, stdv)
            self.nlt_bias.data.uniform_(0, 0) # init as 0
...
class Conv2d_nlt(_ConvNd_nlt):
    ...
    def forward(self, inp):
        ...
        new_weight = self.weight.mul(new_nlt_weight) + new_nlt_bias  # implement neuron linear transformation
