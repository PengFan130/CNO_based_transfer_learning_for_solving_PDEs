...
def weight_decay_loss(model, lamda):
    # Apply loss to constrain nlt weight around 1 and bias around 0.
    loss_weight = 0
    loss_bias = 0
...
folder = "TestModels1/"+"CNO_transfer"+which_example
...
if which_example == "shear_layer":
    ...
    example_nlt = ShearLayer(model_architecture_, device, batch_size, training_samples, size=128, nlt=True, in_dist=False)
    ...
    example_nlt = WaveEquation(model_architecture_, device, batch_size, training_samples, in_dist=False, nlt=True)
    ...
    example_nlt = ApeNS(model_architecture_, device, batch_size, training_samples, in_dist=False, nlt=True)
elif which_example == 'ks':
    example = KS(model_architecture_, device, batch_size, 256)
    example_nlt = KS(model_architecture_, device, batch_size, training_samples, in_dist=False, nlt=True)
elif which_example == 'bruss':
    example = KS(model_architecture_, device, batch_size, 256)
    example_nlt = Bruss(model_architecture_, device, batch_size, training_samples, in_dist=False, nlt=True)
else:
    raise ValueError()

#-----------------------------------Train--------------------------------------
tar_model = example_nlt.model
tar_model.print_size()

# Load source weights into the target model.
sou_model = torch.load(f'TrainedModels6/CNO_ks_l2_512/model.pkl')
tar_model.load_state_dict(sou_model.state_dict(), strict=False)
# tar_model.load_state_dict(torch.load(f"TrainedModels5/CNO_wave_0_5_512/model.pt"), strict=False)
...
for epoch in range(epochs):
    with tqdm(unit="batch", disable=False) as tepoch:
        ...
        for i, (input_t, output_t) in enumerate(tar_train_loader):
            ...
            loss_t = mse_t + weight_decay_loss(tar_model, 1e-4)
        ...
        with torch.no_grad():
            ...
            for step, (input_batch, output_batch) in enumerate(tar_val_loader):
                ...
                loss_val = torch.norm(abs(output_pred_batch - output_batch)) / torch.norm(abs(output_batch)) * 100
            ...
            for step, (input_batch, output_batch) in enumerate(tar_train_loader):
                ...
                loss_f = torch.norm(abs(output_pred_batch - output_batch)) / torch.norm(abs(output_batch)) * 100
